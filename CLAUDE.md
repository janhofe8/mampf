...
├── ViewModels/            RestaurantStore.swift (@Observable), FilterViewModel.swift (debounced search, showMyRatedOnly)
├── Views/                 List (3 Modi: Cards/Grid/List), Detail, Map, Filter, Settings, Profile, Login, EditProfile, Onboarding
├── Components/            Cards, Rating-Bars/Pills/Badges, RatingSlider (custom), SkeletonLoadingView, RatingHistogramFilter, FilterBarChip (+FiltersEntryChip)
...
├── ContentView.swift      TabRouter (@Observable, AppTab enum) injected via env — lets Profile-stats-tiles switch tabs programmatisch (z.B. „Spots erkundet"-Tile → Map mit „Bereits bewertet"-Filter).
...
- **App-Farben:** Purple (.ffPrimary), Lime (.ffSecondary), Charcoal (.ffTertiary), Amber (.ffAmber)
- **Spacing:** 4pt-Grid (4/8/12/16/20/24/28/32) für alle `padding()` und `spacing:` Werte. Sub-4 Werte (1/2/3) nur für typografische Mikro-Spacings (z.B. zwischen 2 Zeilen in einem Label).
- **Brand Voice:** „Jan" kommt **nirgends** im User-facing Text vor. Die App-Persona ist „MAMPF" / „wir" — Jan ist intern Curator, in der UI nicht erwähnt. Gilt für alle Localizable-Strings, Onboarding-Copy, Share-Texts.
...
- **Suche:** Custom Suchbalken in Liste + Map mit `Color(.tertiarySystemBackground)` (im Dark Mode hell genug, dass Card lesbar bleibt) + 1pt `systemGray4` Stroke, 22pt continuous corner radius. Debouncing: `searchText` instant → `activeSearchText` (250ms). List zeigt Dropdown nur bei Query; Map zeigt zusätzlich Küchen-Suggestions bei leerem+fokussiertem Feld.
  ...
  - **Statisches Placeholder-Format** „%d Foodspots durchsuchen…" / „Search %d food spots…" (key: `search.restaurants`). Kein rotierender Placeholder mehr — `RotatingPlaceholder` ist entfernt.
  - **Map-Suchbalken im Fokus:** Lupe links wird zu `chevron.backward` (tappable → schließt Suche), das X rechts ist weg.
- **List-Header Hide-on-Scroll:** `safeAreaInset(edge: .top)` + `frame(height: visible ? nil : 0) + clipped()` mit `.animation(.easeOut(0.22), value: headerVisible)`. Funktioniert in allen 3 View-Modi. `onScrollGeometryChange` füllt nur den `ScrollTracker.accumulator` (Plain Class, siehe Performance-Killer); Visibility-Toggle in `onScrollPhaseChange` und **nur** bei `newPhase == .decelerating` — Slow-Drags enden in `.idle` und ändern nichts. Auto-Show bei `newValue < 20` direkt im Geometry-Callback, gegated auf `phase != .tracking`. Thresholds: 40pt hide, 30pt show. **NICHT conditional im Body-Tree** rendern — zerstört TextField-Identity, Suche bricht beim Tippen ab.
- **Map-Focus-Dismiss nur bei echtem User-Drag:** `regionDidChangeAnimated` feuert auch bei programmatischen `setRegion`-Calls (Search-Result-Tap, Location-Button). Coordinator setzt `isUserInteracting = true` in `UIPanGestureRecognizer`/`UIPinchGestureRecognizer`-Callbacks (state == .began/.changed), reset nach Region-Change-Notify. Flag wird als `userInitiated` an SwiftUI durchgereicht — nur dann Search-Focus + Rating-Histogram dismissen.
- **Filter UX (Liste + Map einheitlich):**
  ...
  - **Liste:** Filters → Sort → Open Now → Bereits bewertet → Cuisine → Preis → Stadtteil. Kein separater Clear-X-Button (Sheet hat Reset-Button).
  - **Map:** Filters → Open Now → Bewertung → Bereits bewertet → Cuisine → Preis (kein Sort/Stadtteil — Pan erfüllt's). **Rating-Chip-Toggle:** aktiv → Tap resettet sofort (nicht Histogram öffnen); inaktiv → Tap öffnet Histogram. „Bewertung"-Label in DE (war „Rating").
  - **„Bereits bewertet"-Filter** (`showMyRatedOnly` in FilterViewModel): toggelt Liste/Map auf Spots die der User selbst bewertet hat. Lebt **nur als Chip** in den Filter-Bars von Map und Liste (keine UI im Filter-Sheet — `FilterSheetView` zieht den State nur in den Hash + `apply(...)`-Count). Chip in beiden Bars gegated auf `!store.myRatings.isEmpty`. `apply(...)` nimmt `myRatedIds: Set<UUID>` als Parameter — wird in MapTabView/RestaurantListView/FilterSheetView aus `store.myRatings.keys` befüllt.
...
- **Map = UIKit `MKMapView` via `MapKitMapView` (`UIViewRepresentable`)**, NICHT SwiftUI `Map`. SwiftUI-`Annotation` rendert pro Frame neue SwiftUI-Views beim Pan/Zoom — bei 80+ Pins + Shadow auf jedem ist das nicht flüssig genug. Wrapper hält `MKMapView` + `MKAnnotationView`-Recycling-Pool, Coordinator implementiert `MKMapViewDelegate`. Region-Sync via `@Binding var region: MKCoordinateRegion` (nicht `MapCameraPosition`) — `updateUIView` ruft `setRegion(animated: true)` nur wenn extern geändert UND nicht User-interagiert (Approximate-Equal-Threshold gegen Feedback-Loop).
- **Pin-Images pre-rendered** (`PinImageCache`, NSLock-thread-safe) via `UIGraphicsImageRenderer` mit eingebackenem Shadow — kein Live-Blur pro Frame mehr. Reuse-Identifier pro Rating-Bucket (`rated-\(Int(r * 2))` für 19 0.5-Steps + `unrated`) damit `dequeueReusableAnnotationView` das richtige Image trifft. Annotation-Diff in `syncAnnotations`: ID-Match + Rating-Equality, neue Rating ⇒ replace (nicht in-place-update, weil `view.image` Cache-bucket nicht ändert sich automatisch).
- **Map-Pin-LOD:** `ZoomTier` (far/medium/close) basiert auf `latitudeDelta`. Weit raus (>0.12): nur Rating ≥ 8. Mittel: nur bewertete Spots. Nah (<0.035): alle. Tier-Wechsel im `onRegionChange`-Callback aus dem Wrapper. **Pins ohne weißen Rand.** Rating ≥ 9 bekommt lila Glow-Shadow direkt im pre-rendered Image (shadowBlur 8, ffPrimary @ 0.65).
- **Map-Pin-Names:** Restaurant-Namen werden als UILabel-Subview unter dem Pin gerendert (SwiftUI→UIKit-Migration hat das früher implizit unterstützt — MKAnnotationView nicht). `attachNameLabel(to:name:)` zentriert das Label uniform `4pt unter visible-pin-bottom` (nicht Image-Bounds — Elite-Pins haben mehr Shadow-Padding). Sichtbarkeit gated auf `region.span.latitudeDelta < 0.012` — nur bei sehr nahem Zoom. View-Setup: `collisionMode = .none` + `displayPriority = .required` + `clusteringIdentifier = nil` damit iOS keine Auto-Clustering/Culling macht. Visibility wird in `regionDidChangeAnimated` für alle Annotation-Views aktualisiert.
- **Map-Fullscreen-Layout:** `MapKitMapView` selbst respektiert die Safe Area (im Gegensatz zu SwiftUI `Map`). Lösung: `MapKitMapView().ignoresSafeArea()` in einer `ZStack` wrappen, alle `.overlay(alignment:)`-Modifier (Search/Chips/Histogram/Location-Button) auf den `ZStack` — so respektieren die Overlays die Safe Area, der Map-Layer extendiert dahinter unter Status-Bar und Tab-Bar. `.overlay` direkt auf den `.ignoresSafeArea()`-Map zu hängen funktioniert NICHT (Overlay erbt das extended Frame und liegt unter der Status-Bar).
- **Image Caching:** Eigener `ImageCache` (NSCache memory + URLCache disk). **NICHT `@MainActor`** — als `final class: @unchecked Sendable`, weil NSCache + URLSession thread-safe sind. So laufen Cache-Writes (besonders Prefetch-Completions) off-main und konkurrieren nicht mit Scroll-Rendering. **Echtes Downsampling** via `CGImageSourceCreateThumbnailAtIndex` im Background-Task — Call-Sites geben `targetSize` mit (× `displayScale`). `totalCostLimit: 80 MB`, `countLimit: 1000` (300 evictete bei 6 Bucket-Sizes × 155 Spots zu aggressiv → Cache-Hits wurden zu Misses bei Scroll-Back).
  ...
  - **Foodie-Preset für `/own/`-URLs:** `CIGammaAdjust` (adaptive Helligkeit pro Bild, Ziel-Luminanz 0.45, gamma clamped 0.55–1.25) + `CIHighlightShadowAdjust` (Highlights 0.65, Shadows 0.25) + `CITemperatureAndTint` (+150K) + `CIColorControls` (Saturation 1.13, Contrast 1.03). Google-Photos bleiben unberührt. **`CIContext` als shared `static let`** — pro-Call-Allocation alloziert Metal-Queues + kompiliert Shader, war messbar im Decode-Burst beim Cold-Start. Thread-safe für concurrent renders.
...
- **Profile-Stats-Card:** Top-Spot Wide Tile (Bild + Name + Rating-Pill rechts) + 2×2 Grid mit 4 Tiles (Spots erkundet · MAMPFs entdeckt X/Y · Stammviertel · Lieblingsküche). Alle Tiles tappable via `StatTileButtonStyle` (custom ButtonStyle: scaleEffect 0.97 + opacity 0.85 on press). Tap-Aktionen via `TabRouter.selected = .map` + Filter-Mutation: Erkundet→`showMyRatedOnly` (Tile zeigt ja % bewertet, also Map gefiltert auf eigene Bewertungen — `disabled` wenn `store.myRatings` leer), MAMPFs→`minimumRating=9`, Stammviertel→`selectedNeighborhoods`, Lieblingsküche→`selectedCuisines`, Top-Spot→`navigatedRestaurant`. Lila-Tint-Background (`Color.ffPrimary.opacity(0.08)`) hebt Stats-Region von neutralen Listen-Sections drunter ab.
  - **Locked-State (signed-out):** Stats-Card geblurrt (`blur(radius: 8)`) mit Lila-Capsule-CTA „Anmelden für personalisierte Stats" oben drauf. Demo-Werte fürs Visual (12/8%/3/Top-Spot/Ottensen/🍝). Tap auf CTA: holt Banner-Email rüber via `prefilledEmailForSheet` und öffnet Login-Sheet. Lila-CTA ist die einzige tappable Schicht (`allowsHitTesting(false)` auf der geblurrten Stats darunter).
  - **MAMPFs-Stat = X/Y** wo X = wie viele Spots du bewertet hast bei denen MAMPF-Rating ≥9 ist (also „Jans MAMPFs entdeckt"), Y = total MAMPFs in DB. Implementiert als `mampfsCount` + `jansMampfsTotal` in `recomputeStats()`. Daten werden gecached, nicht pro Render berechnet (siehe Performance-Killer).
  - **Avatar:** Erster Buchstabe des Display-Namens (oder Email-Local-Part bei Anon) als weiße rounded-black 26pt Schrift in lila 56×56 Circle. Helper: `userInitial`.
...
- **`deleteAllData` darf `DeviceID.reset()` NIE aufrufen wenn der Server-Delete fehlgeschlagen ist** — sonst werden die anon Ratings auf dem Server (`device_id`-keyed) für immer unerreichbar, weil das Gerät beim nächsten Launch eine neue Device-ID bekommt. Bei Server-Fehler `return` direkt nach dem `catch`, lokalen State unangetastet lassen damit der User retryen kann.

## Onboarding

- **4 Pages, full-color brand-Backgrounds + große Typografie** (46pt black-rounded für Headlines, title3 semibold für Subtitles). Pages: Welcome (Lila, 🌮🍔🍝 collage) → Expertise (Lime, „Genug von Fake-Reviews? / Wir auch.") → Rating (Grey, 3 Source-Pills + Tier-Legende) → Personalize (Amber, Avatar + Mini-Stats-Preview, „Werde selbst zum Foodscout."). Kein Location-Permission-Page mehr — moved to in-context flow auf der Map.
- **Color-Tour:** bgColor + fgColor wechseln pro Page (computed properties in `OnboardingView`). Brand-Backgrounds: Lila/Lime/Grey/Amber. Foreground white auf dunkel/lila, charcoal auf hellem Background. Buttons flippen entsprechend (white-pill mit lila/charcoal Text).
- **Bottom-Button reserviert konstante Höhe:** „Weiter" auf 0-2, „Foodspots entdecken" / „Explore food spots" auf Page 3 (last). Kein Skip-Button mehr — alles inline.
- **In-Context Location-Permission auf der Map:** Wenn `authorizationStatus == .notDetermined` und `hasSeenLocationPrimer` (`@AppStorage`) nicht gesetzt: nach 800ms ein Bottom-Sheet-Primer (`presentationDetents([.medium])`) mit 🗺️ + „Spots in deiner Nähe" + Erklärung + lila „Standort aktivieren"-Button + „Jetzt nicht"-Skip. Allow → triggert iOS-System-Prompt + setzt Flag. Skip → setzt nur Flag, Location-Button auf der Map fragt bei nächstem Tap weiter (existing flow).
...
- **Profil-Tab:** Zeigt **unterschiedliche Strukturen für anon vs signed-in** — Stats sind bei Anon hinter Locked-Overlay (siehe „Profile-Stats-Card"). „Your Ratings" + „Want to Try" Listen sind bei beiden sichtbar (Anon Ratings via `device_id`, Wishlist aus UserDefaults). Oben:
  - Signed-in: Account-Header (Avatar mit Initialbuchstabe + Name + „Profil bearbeiten" Subtitle, tap → Edit Profile)
  - Signed-out: Inline Email-Entry-Card unter Heading „Werde ein Teil der MAMPF-Community" + Arrow-Button (lila wenn valid email, sonst grau). Submit → LoginSheet mit `prefilledEmail` → skippt Email-Step direkt zu Password
  - Settings via Gear oben rechts. **Sign Out lebt jetzt in Settings**, nicht mehr im Profile-Tab (zusammen mit Delete-Account in einer „Account"-Section, kombiniert)
...
- **Performance-Killer die wir gefunden haben:**
  ...
  - **Animated `safeAreaInset` height-change WÄHREND aktivem Drag** → iOS justiert `contentOffset` zur Kompensation, Liste folgt nicht mehr 1:1 dem Finger, fühlt sich an als „springt der Scroll alleine". Fix: Visibility-Toggle aus dem Geometry-Callback ziehen, in `onScrollPhaseChange` schieben und nur auf `newPhase == .decelerating` reagieren — Slow-Drags enden in `.idle` ohne Phasenwechsel und triggern nichts. Phase-Gating allein (`phase != .tracking` im Geometry-Callback) reicht NICHT — feuert zu früh wenn der Finger lift während Accumulator über Threshold ist.
  - **Opening-Hours-Parser pro Row-Render mehrfach aufgerufen:** `restaurant.closingSoon` + `restaurant.minutesUntilClosing` + `restaurant.isOpenNow` parsen jeweils den vollen `openingHours`-String (Trim, Unicode-Normalize via 4× `replacingOccurrences`, Segment-Split, Day-Pattern-Match) → 3 Parses pro Row, bei jedem Scroll-Event neu sichtbare Rows. Fix: `OpeningHoursParser.openingInfo(for:)` als Single-Pass-Variante (Status + minutesUntilClosing in einem Durchgang), `Restaurant.openingInfo` als bevorzugter Hot-Path-Accessor in `listRow` und `RestaurantCardView`.
