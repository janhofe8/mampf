...
- **List-Header Hide-on-Scroll:** `safeAreaInset(edge: .top)` + `frame(height: visible ? nil : 0) + clipped()` mit `.animation(.easeOut(0.22), value: headerVisible)`. **Funktioniert in allen 3 View-Modi** (Cards/Grid/List). Trigger über `.onScrollGeometryChange`, Thresholds 40pt (hide) / 30pt (show). **Accumulator MUSS in Plain Class (`ScrollTracker`)** — als `@State CGFloat` invalidiert er den Body pro Scroll-Event, was Scroll-Stutter verursacht. Nur `isHeaderVisible` (was tatsächlich UI driven) bleibt `@State`. **NICHT conditional im Body-Tree** — das zerstört TextField-Identity und die Suche "bricht sofort ab" beim Tippen.
