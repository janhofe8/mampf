...
├── ContentView.swift      TabRouter (@Observable, AppTab enum) injected via env — lets Profile-stats-tiles switch tabs programmatisch (z.B. Bewertet-Tile → Map mit „Meine Bewertungen"-Filter).
...
- **Filter UX (Liste + Map einheitlich):**
  ...
  - **Liste:** Filters → Sort → Open Now → Meine Bewertungen → Cuisine → Preis → Stadtteil. Kein separater Clear-X-Button (Sheet hat Reset-Button).
  - **Map:** Filters → Open Now → Bewertung → Meine Bewertungen → Cuisine → Preis (kein Sort/Stadtteil — Pan erfüllt's). **Rating-Chip-Toggle:** aktiv → Tap resettet sofort (nicht Histogram öffnen); inaktiv → Tap öffnet Histogram. „Bewertung"-Label in DE (war „Rating").
  - **„Meine Bewertungen"-Filter** (`showMyRatedOnly` in FilterViewModel): toggelt Liste/Map auf Spots die der User selbst bewertet hat. Chip nur sichtbar wenn `store.myRatings` nicht leer. `apply(...)` nimmt `myRatedIds: Set<UUID>` als Parameter — wird in MapTabView/RestaurantListView/FilterSheetView aus `store.myRatings.keys` befüllt.
...
- **Profile-Stats-Card:** Top-Spot Wide Tile (Bild + Name + Rating-Pill rechts) + 2×2 Grid mit 4 Tiles (Spots erkundet · MAMPFs entdeckt X/Y · Stammviertel · Lieblingsküche). Alle Tiles tappable via `StatTileButtonStyle` (custom ButtonStyle: scaleEffect 0.97 + opacity 0.85 on press). Tap-Aktionen via `TabRouter.selected = .map` + Filter-Mutation: Bewertet→`showMyRatedOnly`, Erkundet→clear, MAMPFs→`minimumRating=9`, Stammviertel→`selectedNeighborhoods`, Lieblingsküche→`selectedCuisines`, Top-Spot→`navigatedRestaurant`. Lila-Tint-Background (`Color.ffPrimary.opacity(0.08)`) hebt Stats-Region von neutralen Listen-Sections drunter ab.
...
- **Debug-Reset:** `MAMPFApp.init()` ruft unter `#if DEBUG` `UserDefaults.standard.removeObject(forKey: "hasCompletedOnboarding")` — bei jedem Cold-Launch erscheint Onboarding wieder. In Release-Builds normal (einmalig).
